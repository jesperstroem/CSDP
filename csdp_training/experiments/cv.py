...
from csdp_pipeline.pipeline_elements.samplers import Determ_sampler, Random_Sampler, SamplerConfiguration
...
class CV_Experiment:
    def __init__(
        ...
        if split_filepath is not None:
            ...
            self.split_data: [Split] = self.__create_split(
                self.dataset_paths, num_folds=num_folds, num_validation_subjects=num_validation_subjects
            )
    ...
    def run(self):
        if self.test_first:
            ...
            trainer = self.__init_trainer(
                max_epochs=self.training_epochs, split_data=global_split, split_name="Global Test"
            )
        ...
        for _, split in enumerate(self.split_data):
            ...
            trainer = self.__init_trainer(max_epochs=self.training_epochs, split_data=split, split_name=split_name)

            base_net = deepcopy(self.base_net)

            net, trainer = self.__train(base_net, wrapper, trainer)

            self.__test(trainer, wrapper, net=net, split_name=split_name)

    def __train(self, net: USleep_Lightning, wrapper: Dataloader_Factory, trainer: pl.Trainer):
    ...
    def __init_trainer(self, max_epochs: int, split_data: Split, split_name: str) -> pl.Trainer:

        checkpoint_callback = ModelCheckpoint(
            dirpath=f"{self.logging_folder}/weights", filename=f"best-{split_name}", monitor="valKap", mode="max"
        )

        early_stopping = EarlyStopping(
            monitor="valKap", min_delta=0.00, patience=self.earlystopping_patience, verbose=True, mode="max"
        )
    ...
    def __create_wrapper(self, split: Split, batch_size):

        train_sampler = Random_Sampler(split, num_epochs=35, num_iterations=batch_size * self.batches_per_epoch)

        val_sampler = Determ_sampler(split, get_all_channels=self.pick_all_channels[1], split_type="val")

        test_sampler = Determ_sampler(split, get_all_channels=self.pick_all_channels[2], split_type="test")

        samplers = SamplerConfiguration(train_sampler, val_sampler, test_sampler)

        pipes = self.pipeline_configuration

        wrapper = Dataloader_Factory(batch_size, samplers, pipes)

        return wrapper

    def __create_split(self, dataset_filepaths: list[str], num_folds, num_validation_subjects=1) -> [Split]:
        all_subs = []
        ...
        for i, (train_index, test_index) in enumerate(kf.split(all_subs)):
            split_data = Split(id=i, dataset_splits=[], base_data_path=self.base_data_path)

            train_records = [all_subs[i] for i in train_index]

            train_records, val_records = train_test_split(train_records, test_size=num_validation_subjects)

            test_records = [all_subs[i] for i in test_index]

            for file in dataset_filepaths:
                dataset_split = Dataset_Split(file, train=[], val=[], test=[])
    ...
    def __create_global_split(self, dataset_filepaths: list[str]):
        split_data = Split(id="test", dataset_splits=[], base_data_path=self.base_data_path)
        ...
        for dataset_filepath in dataset_filepaths:
            ...
            split_data.dataset_splits.append(Dataset_Split(dataset_filepath, [], [], subs))
