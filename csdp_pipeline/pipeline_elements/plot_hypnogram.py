...
def plotHypnoGram(
    ...
    labels = [transformdic[lbl] for lbl in labels]

    labels = np.array(labels)

    if datetimeList is None:
        recordingStart = np.datetime64("2023-01-01")
        datetimeList = np.datetime64(recordingStart).astype(
            "datetime64[s]"
        )  # convert to datenum with seconds precision
        datetimeList = np.array(
            datetimeList + np.arange(0, len(labels)) * 30, dtype="datetime64[s]"
        )  # convert to array
