...
class RegionalAugmenter(ABC):
    ...
    def get_aug_length(self, x_length):
        ...
        aug_length = int(
            np.random.uniform(min_idx, max_idx)
        )  # Random length between min and max (but should 0 be included?
...
class Augmenter(IPipe):
    def __init__(self, min_frac, max_frac, apply_prob, sigma, mean):
        ...
        self.regional_gaussian_noise = RegionalGaussianNoise(
            self.min_frac, self.max_frac, self.apply_prob, self.sigma, self.mean
        )
