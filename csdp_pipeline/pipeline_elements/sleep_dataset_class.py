...
from ..preprocessing.usleep_prep_steps import filter_channel, clip_channel, scale_channel, resample_channel, remove_dc
...
class sleep_dataset_from_paths(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, EEG_paths,L=1,scoring_paths=[],
        ch_names=None,derivations=None,scoring_preprocess=None,sdcFile=None,fullRecords=False):
        if sdcFile is None:
    ...
    def constructFromPaths(self, EEG_paths,L,scoring_paths=[], ch_names=None, derivations=None,scoring_preprocess=None,fullRecords=False,passNanEpochs=False):
                ...
                for didx,deriv in enumerate(tempDerivs):
                    if deriv is None:
                        ...
                        if np.isscalar(deriv)==1:
                            ...
                            raise ValueError('Each derivation should either be single channel ID or a tuple of two lists of channels. \n You passed '+str(deriv))
    ...
    def extract_nansamples(self):
        ...
        for nansamples in nansamples_list: #loop over all recordings
            ...
            for iChannel in range(nansamples.shape[0]):
                ...
                nanEpochs[iChannel,:]=np.all(nansamples[iChannel,:(nEpochs*self.epochLength)].
                                             reshape(self.epochLength,-1,order='f'),axis=0).reshape(1,-1)
...
if __name__ == '__main__':
    ...
    dataset=sleep_dataset_from_paths(filePaths,
                                    derivations=[(['EL1','EL2','EL3','EL4','EL5'],['ER1','ER2','ER3','ER4','ER5'])],fullRecords=True)
