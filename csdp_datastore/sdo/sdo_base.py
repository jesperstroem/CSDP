...
class SleepdataOrg(BaseDataset):
    ...
    def list_records(self, basepath):
        ...
        for idx in range(len(psg_files)):
            ...
            hyp_file_path = psg_file_path.replace("/" + psg + "/", "/" + hyp + "/", 1).replace(
                ".edf", "-profusion.xml", 1
            )
            ...
            if not labels_exist:
                self.log_warning("File does not exist, skipping this record")
                continue
    ...
    def read_psg(self, record):
        ...
        for channel in self.channel_mapping().keys():
            ...
            assert len(final_channel_data) == len(y) * sample_rate * 30, (
                f"Channel length was {len(final_channel_data)}, but according to the number of labels it should be {len(y) * sample_rate * 30}. Check the sample rate or override slice_channels if needed."
            )
